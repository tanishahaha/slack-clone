...
export const createWorkspace=async({imageUrl, name, slug, invite_code}:{imageUrl?:string;name:string;slug:string;invite_code:string;})=>{
    ...
    const [updateWorkspaceData,updateWorkspaceError] =await updatUserWorkspace(userData.id,workspacerecord[0].id);
    ...
    const [addMemberWorkspaceData,addMemberWorkspaceError] = await addMemberToWorkspace(userData.id,workspacerecord[0].id);
